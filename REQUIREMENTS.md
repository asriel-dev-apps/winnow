...
- **インラインアクション（オーナーのみ）**: ロード時に `GET /api/feedback/state` の成否で判定し、成功時のみ各カードに「★お気に入り / ✕興味なし」ボタンをJSで注入する。閲覧者には操作UIを一切表示しない（`file://` はオーナー扱い）
  ...
  - **長押しドラッグジェスチャー**【使用感の再検討のため一時無効化中。テンプレートの `DRAG_GESTURE` フラグで再有効化可】（タッチのみ、オーナーのみ）: カードを**350ms長押しで持ち上げてから**右ドラッグ=★ / 左ドラッグ=✕（ボタンと同じトグル動作、確定閾値100px）。持ち上げ後は指に1:1追従（±160px超はラバーバンド）し、離すとスプリングで復帰。**長押しなしの横スワイプには一切反応しない**ためスクロールと完全に非干渉（持ち上げ前に10px動いたらキャンセル、持ち上げ中はtouchmoveをpreventDefaultしてスクロールをロック）。リンク・ボタン上では開始せず、長押し中のテキスト選択・コンテキストメニューは抑止
