...
- **インラインアクション（オーナーのみ）**: ロード時に `GET /api/feedback/state` の成否で判定し、成功時のみ各カードに「★お気に入り / ✕興味なし」ボタンをJSで注入する。閲覧者には操作UIを一切表示しない（`file://` はオーナー扱い）
  ...
  - **スワイプジェスチャー**（タッチのみ、オーナーのみ）: カードを右スワイプ=★ / 左スワイプ=✕（ボタンと同じトグル動作）。`touch-action: pan-y` で縦スクロールはブラウザに委ね、横意図が明確（|dx|>28 かつ |dx|>1.6|dy|）になってからのみ介入。発火閾値は110px、リンク・ボタン上では開始しない
